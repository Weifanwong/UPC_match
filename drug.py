...
for j in range(6,7):
	for i in range(0,len(tmp1)):
		...
		for ele in UPC:  #去掉所有位数大于10的UPC码
			if len(str(ele))>10:
				UPC.remove(ele)
		upc_length = len(UPC)		
#---------------------------开始匹配药店----------------------------
		for test_upc in UPC:
			...
			fileName2 = open('G:\\数据\\Academic Dataset External copy\\'+year_list[j]+'\\External\\'+tmp2[i]+'\\'+tmp2[i]+'_drug_1427_1478')  #销售记录文档
			...
			if (xlsheet.cell(row=2,column=1).value != None):
				isExists = os.path.exists('G:\\数据\\result\\'+year_list[j]+'\\'+tmp2[i]+'\\drug')
				if not isExists:
					os.makedirs('G:\\数据\\result\\'+year_list[j]+'\\'+tmp2[i]+'\\drug')
					book_drug.save('G:\\数据\\result\\'+year_list[j]+'\\'+tmp2[i]+'\\drug\\'+year_list[j]+'-'+tmp2[i]+'drug'+'-'+str(test_upc)+'.xlsx')
				else:
					book_drug.save('G:\\数据\\result\\'+year_list[j]+'\\'+tmp2[i]+'\\drug\\'+year_list[j]+'-'+tmp2[i]+'drug'+'-'+str(test_upc)+'.xlsx')
		fileName2.close();
