...
#---------------------------开始匹配杂货店----------------------------
		...
		for test_upc in UPC:
					...
					fileName3 = open('G:\\数据\\Academic Dataset External copy\\'+year_list[j]+'\\External\\'+tmp3[i]+'\\'+tmp2[i]+'_groc_1114_1165')  #销售记录文档
					...
					if (xlsheet.cell(row=2,column=1).value != None):
						...
						if not isExists:
							...
							book_groc.save('G:\\数据\\result\\'+tmp2[i]+'\\groc\\'+year_list[j]+'-'+tmp2[i]+'groc'+'-'+str(test_upc)+'.xlsx')
						else:
							book_groc.save('G:\\数据\\result\\'+tmp2[i]+'\\groc\\'+year_list[j]+'-'+tmp2[i]+'groc'+'-'+str(test_upc)+'.xlsx')
		fileName3.close();