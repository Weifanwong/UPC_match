...
#---------------------------开始匹配杂货店----------------------------
		...
		for test_upc in UPC:
					...
					if (xlsheet.cell(row=2,column=1).value != None):
						isExists = os.path.exists('G:\\数据\\result1\\'+year_list[j]+'\\'+tmp2_add[i]+'\\drug')
						if not isExists:
							os.makedirs('G:\\数据\\result1\\'+year_list[j]+'\\'+tmp2_add[i]+'\\drug')
							book_drug.save('G:\\数据\\result1\\'+year_list[j]+'\\'+tmp2_add[i]+'\\drug\\'+year_list[j]+'-'+tmp2_add[i]+'drug'+'-'+str(test_upc)+'.xlsx')
						else:
							book_drug.save('G:\\数据\\result1\\'+year_list[j]+'\\'+tmp2_add[i]+'\\drug\\'+year_list[j]+'-'+tmp2_add[i]+'drug'+'-'+str(test_upc)+'.xlsx')
		fileName3.close();